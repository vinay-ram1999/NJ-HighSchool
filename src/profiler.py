...
tables = ["fct_enrollments", "fct_homelang_enrollments"]

for table in tables:
    df = pl.read_csv(f"{nj_doe_data_dir}/{table}.csv")
    ...
    plt.title(f"Percentage Fullness Heatmap for '{table}'")
    plt.xlabel("Academic Year")
    plt.ylabel("Attribute")

    plt.tight_layout()
    plt.savefig(f"{nj_doe_data_dir}/{table}_heatmap.png", dpi=300, bbox_inches='tight')
    plt.show()
